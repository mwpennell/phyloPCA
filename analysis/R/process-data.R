...
phy.car$edge.length <- phy.car$edge.length/max(branching.times(phy.car))

pc <- princomp(bio.mean, cor =TRUE)
ppc <- phyl.pca(phy.car, bio.mean, mode="corr")

bioclim.dat <- list(tree=phy.car, raw=apply(bio.mean, 2, function(x) x/sd(x)), pc=pc$scores, ppc=ppc$S, pcall=pc, ppcall=ppc)
