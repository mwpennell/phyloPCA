...
get.contrasts <- function(dat, simmodel){
  btimes <- lapply(1:length(dat), function(x) 1 - branching.times(dat[[x]]$tree))
  picraw <- lapply(1:length(dat), function(x) sapply(1:ntraits, function(y) pic(dat[[x]]$raw[,y], dat[[x]]$tree)))
  picpc <- lapply(1:length(dat), function(x) sapply(1:ntraits, function(y) pic(dat[[x]]$pc[,y], dat[[x]]$tree)))
  picppc <- lapply(1:length(dat), function(x) sapply(1:ntraits, function(y) pic(dat[[x]]$ppc[,y], dat[[x]]$tree)))
  ...
  contmelt <- melt(contdat, id=c("type", "times"))
