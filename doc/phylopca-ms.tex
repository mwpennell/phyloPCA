...
\[\text{AICw}_i = \frac{e^{0.5(\text{AIC}_{min} - \text{AIC}_i)}}{\sum_j e^{0.5(\text{AIC}_{min} - \text{AIC}_j)}}\]
and therefore if the likelihoods are identical, OU and EB will have a $\Delta \text{AIC} = \text{AIC}_{min} - \text{AIC}_i = 2$ (as OU and EB each have one more parameter than BM). The theoretical maximum for the AICw of BM is thus $1/(2e^{-1} + 1) \approx 0.576$.
